...
fileHandler = TimedRotatingFileHandler(filename=f'{default_path}/logs/forecast_wave.log', when='midnight', interval=1, encoding='utf-8')
...
def job_wave():
    # base url
    #base = 'https://coastwatch.pfeg.noaa.gov/erddap/griddap/NWW3_Global_Best.nc?'
    base = 'https://pae-paha.pacioos.hawaii.edu/erddap/griddap/ww3_global.nc?' # alternative
              
    
    # 시간 차원
    # 해당일 12:00 ~ 해당일+4일 18:00 (UTC 기준)
    # 20251021 Edited By Shipman98. NOAA NWW3 Data release date 변경 4 days --> 3 days
    dateformat="%Y-%m-%dT%H:%M:%SZ"
    now = datetime.datetime.utcnow()
    days = 3  # 실제로는 3일치임. 
    # end = now+datetime.timedelta(days=days)    
    #start =  datetime.datetime.combine(now, datetime.time(hour=12)).strftime(dateformat)
    start =  datetime.datetime.combine(now, datetime.time(hour=18)).strftime(dateformat)    
    stop =   datetime.datetime.combine(now+datetime.timedelta(days=days), datetime.time(hour=18)).strftime(dateformat)    
    ...
    # grid_vars = ['Tdir', 'Tper', 'Thgt', 'sdir', 'sper', 'shgt', 'wdir', 'wper', 'whgt']
    
    dimensions = f'%5B({start}):1:({stop})%5D%5B(0.0):1:(0.0)%5D%5B({lat_min}):1:({lat_max})%5D%5B({lon_min}):1:({lon_max})%5D'
    
    dateformat2="%Y-%m-%d_%H" # for filename
    # fn = f'noaa_nww3_{days}d_{now.strftime(dateformat2)}.nc'
    
    logger.info(f'start wave data download: {start} ~ {stop}')        
    
    # Total(swell + wind wave)
    grid_vars = ['Tdir', 'Tper', 'Thgt']
    fn = f'{default_path}/data/noaa_nww3_T_{days}d_{now.strftime(dateformat2)}.nc'
    if download_urlretrieve(base, grid_vars, dimensions, fn):
        send_request_to_update('twave', fn)
    
    # swell
    grid_vars = ['sdir', 'sper', 'shgt']
    fn = f'{default_path}/data/noaa_nww3_s_{days}d_{now.strftime(dateformat2)}.nc'
    if download_urlretrieve(base, grid_vars, dimensions, fn):
        send_request_to_update('swell', fn)
    
    # wind wave
    grid_vars = ['wdir', 'wper', 'whgt']
    fn = f'{default_path}/data/noaa_nww3_w_{days}d_{now.strftime(dateformat2)}.nc'
    if download_urlretrieve(base, grid_vars, dimensions, fn):
        send_request_to_update('wwave', fn)
