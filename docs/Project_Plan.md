...
> **최초 작성일**: 2026-03-15 (KST) | **최종 수정일**: 2026-03-16 (KST, Phase 7 완료)
...
### 1-2. 수집 데이터 종류 — 예보(미래) ✅ 개발 완료 (실행 테스트 미완료)

| 데이터 | 출처 | 변수 | 해상도 | 예보 기간 |
|--------|------|------|--------|----------|
| 바람 예보 | ECMWF Open Data (ecmwf-opendata 라이브러리) | u10, v10 (ERA5와 동일) | 0.25° | 10일 |
| 파랑 예보 | ECMWF Open Data | swh, mwd, mwp 등 (ERA5와 동일) | 0.25° | 10일 |
...
### 4-1. 현재 구조 (✅ Phase 6 완료)
```
llm_for_ship/
├── config/
│   ├── settings.toml          # 파이프라인 설정 (예보 섹션 포함)
...
│   │   │   └── ecmwf_wave_YYYYMMDD.nc
│   │   └── forecast/YYYY/MM/
│   │       ├── ecmwf_fc_wind_YYYYMMDD.nc   (발행일 기준 파일명)
...
├── specs/
│   ├── ecmwf_reanalysis.md
│   ├── ecmwf_forecast.md      # ✅ 신규
│   ├── hycom_forecast.md      # ✅ 신규
│   └── db_schema.md
│
├── src/env_pipeline/
│   ├── pipeline.py            # ✅ 6가지 mode 지원
│   ├── ecmwf/
│   │   ├── era5_downloader.py
│   │   └── ecmwf_forecast_downloader.py  # ✅ 신규
│   ├── hycom/
│   │   ├── hycom_downloader.py
│   │   └── hycom_forecast_downloader.py  # ✅ 신규
│   └── db/
│       ├── connection.py
│       ├── schema.py          # ✅ 예보 테이블 2개 추가
│       └── loader.py          # ✅ 예보 파일 패턴 + issued_at 처리 추가
│
├── run.py                     # ✅ 6가지 mode 지원
├── pyproject.toml             # ✅ ecmwf-opendata, cfgrib, eccodes 추가
├── docker-compose.yml
└── .env
```
...
- ECMWF Open Data wave: `shts, mdts, mpts, shww, mdww, mpww` 6개 변수 **미제공** (무료 tier 제한)
  → DB에서 해당 컬럼은 NULL, LLM Agent에서 NULL 처리 필요
...
### 7-2. 추가된 테이블 (예보) ✅ 구현 완료 (실행 테스트 완료)

```sql
-- ECMWF 예보 (바람 + 파랑 통합)
-- issued_at: 예보 발행 시각 (언제 만들어진 예보인지)
-- valid_time → datetime 컬럼: 실제 해당하는 시각
CREATE TABLE env_ecmwf_forecast (
    issued_at TIMESTAMPTZ NOT NULL,     -- 예보 발행일 (오늘 날짜의 00UTC 또는 12UTC)
    datetime  TIMESTAMPTZ NOT NULL,     -- 예보가 나타내는 실제 시각
    lat       REAL        NOT NULL,
    lon       REAL        NOT NULL,
    ...
    shww REAL, mdww REAL, mpww REAL,
    u10 REAL, v10 REAL,
    PRIMARY KEY (issued_at, datetime, lat, lon)  -- 같은 시각이라도 발행일 다르면 다른 행
);
...
### 8-2. 예보 파이프라인 (✅ 코드 완료, 실행 테스트 미완료)
```bash
uv run python run.py --mode forecast               # 예보 다운로드 + DB 적재
uv run python run.py --mode forecast_download_only # 예보 다운로드만
uv run python run.py --mode full_with_forecast     # 재분석 + 예보 동시 실행
```
...
#### Step 3. 버그 수정 후 재테스트
- [ ] `down.json` 날짜 안전 범위로 재설정: `end_date`를 오늘 -7일 이전으로 설정
  - 예: `{"start_date": "2026-03-01", "end_date": "2026-03-09"}`
- [ ] `uv run python run.py --mode download_only` 재실행 → 경도 변환 확인
  - ECMWF 파일의 lon이 -180~180인지 확인 (`scripts/check_forecast_vars.py` 활용)
- [ ] HYCOM 분석 날짜 재확인 (서버에 있는 최신 날짜 확인 후 down.json 조정)
- [ ] Docker 기동 → `uv run python run.py --init-db` → DB 스키마 4개 테이블 생성 확인
- [ ] `uv run python run.py --mode load_only` → DB 적재 확인
- [ ] `uv run python run.py --mode forecast` → 예보 DB 적재 확인

### 중기
- [ ] **스케줄러**: 매일 오전 8시 KST 자동 실행 (APScheduler 또는 cron)
- [ ] **ERA5T 명시적 수집**: CDS API `product_type` 파라미터 조정 검토
- [ ] **Spec 문서**: `specs/hycom_current.md` 작성
...
| 예보 바람·파랑 출처 | ECMWF Open Data (ecmwf-opendata) | ERA5와 동일 변수 → DB 스키마 재사용 |
